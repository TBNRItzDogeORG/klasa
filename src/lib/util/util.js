...
class Util {
  ...
  static chunk(entries, chunkSize) {
    ...
    if (!Number.isInteger(chunkSize))
      throw new TypeError("chunkSize is not an integer.");
  ...
  static mergeObjects(objTarget = {}, objSource) {
    for (const key in objSource)
      objTarget[key] = Util.isObject(objSource[key])
        ? Util.mergeObjects(objTarget[key], objSource[key])
        : objSource[key];
    return objTarget;
  }
  ...
  static deepClone(source) {
    ...
    if (Util.isObject(source)) {
      ...
      for (const [key, value] of Object.entries(source))
        output[key] = Util.deepClone(value);
      return output;
    }
    if (source instanceof Map) {
      const output = new source.constructor();
      for (const [key, value] of source.entries())
        output.set(key, Util.deepClone(value));
      return output;
    }
  ...
  static isThenable(input) {
    ...
    return (
      ...
      (input !== Promise.prototype &&
        Util.isFunction(input.then) &&
        Util.isFunction(input.catch))
    );
  }
  ...
  static mergeDefault(def, given) {
    ...
    for (const key in def) {
      if (typeof given[key] === "undefined")
        given[key] = Util.deepClone(def[key]);
  ...
  static resolveGuild(client, guild) {
    ...
    if (type === "object" && guild !== null) {
      ...
      if (guild instanceof GuildChannel || guild instanceof Message)
        return guild.guild;
