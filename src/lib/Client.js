...
class KlasaClient extends Discord.Client {
  ...
  constructor(options = {}) {
    ...
    const { guilds, users, clientStorage, members } = this.options.gateways;
    const guildSchema =
      "schema" in guilds ? guilds.schema : this.constructor.defaultGuildSchema;
    const userSchema =
      "schema" in users ? users.schema : this.constructor.defaultUserSchema;
    const clientSchema =
      "schema" in clientStorage
        ? clientStorage.schema
        : this.constructor.defaultClientSchema;
    const memberSchema =
      "schema" in members
        ? members.schema
        : this.constructor.defaultMemberSchema;

    this.gateways.members = new MemberGateway(
      this.gateways,
      "members",
      memberSchema,
      members.provider || this.options.providers.default
    );
    this.gateways.keys.add("members");
    this.gateways._queue.push(
      this.gateways.members.init.bind(this.gateways.members)
    );

    // Update Guild Schema with Keys needed in Klasa
    const prefixKey = guildSchema.get("prefix");
    if (!prefixKey || prefixKey.default === null) {
      guildSchema.add("prefix", "string", {
    ...
    if (!languageKey || languageKey.default === null) {
      guildSchema.add("language", "language", {
        default: this.options.language,
      });
    }

    guildSchema.add("disableNaturalPrefix", "boolean", {
   ...
   */
  get invite() {
    const permissions = new Permissions(this.constructor.basePermissions).add(
      ...this.commands.map((command) => command.requiredPermissions)
    ).bitfield;
  ...
  validatePermissionLevels() {
    const permissionLevels =
      this.options.permissionLevels || this.constructor.defaultPermissionLevels;
    if (!(permissionLevels instanceof PermissionLevels))
      throw new Error(
        "permissionLevels must be an instance of the PermissionLevels class"
      );
  ...
  async login(token) {
    ...
    const loaded = await Promise.all(
      this.pieceStores.map(
        async (store) => `Loaded ${await store.loadAll()} ${store.name}.`
      )
    ).catch((err) => {
      console.error(err);
      process.exit();
  ...
  sweepMessages(
    ...
    commandLifetime = this.options.commandMessageLifetime
  ) {
  ...
  static use(mod) {
    ...
    if (typeof plugin !== "function")
      throw new TypeError(
        "The provided module does not include a plugin function"
      );
    plugins.add(plugin);
    return this;
  }
...
KlasaClient.defaultPermissionLevels = new PermissionLevels()
  ...
  .add(
    6,
    ({ guild, member }) => guild && member.permissions.has(FLAGS.MANAGE_GUILD),
    { fetch: true }
  )
