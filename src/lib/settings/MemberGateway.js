...
class MemberGateway extends GatewayStorage {
  ...
  create(id, data = {}) {
    ...
    const settings = new this.Settings(this, {
      id: `${guildID}.${memberID}`,
      ...data,
    });
   ...
   */
  async sync(
    input = this.client.guilds.reduce(
      (keys, guild) =>
        keys.concat(guild.members.cache.map((member) => member.settings.id)),
      []
    )
  ) {
    if (Array.isArray(input)) {
      ...
      for (const guild of this.client.guilds.values()) {
        for (const member of guild.members.cache.values())
          if (member.settings._existsInDB !== true)
            member.settings._existsInDB = false;
      }
      return this;
    }
    ...
    if (!target)
      throw new TypeError(
        "The selected target could not be resolved to a string."
      );
